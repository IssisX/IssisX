...
func _build_visual() -> void:
    var trolley := GeomUtil.box_mesh(
        Vector3(2.2, 0.48, 1.15),
        Color(0.42, 0.30, 0.075),
        0.74,
        0.24
    )
    ...
    var load := GeomUtil.box_mesh(
        Vector3(2.8, 1.25, 1.75),
        Color(0.25, 0.17, 0.075),
        0.86,
        0.16
    )
    ...
    for side in [-1.12, 1.12]:
        var strap := GeomUtil.box_mesh(
            Vector3(0.10, 1.35, 1.80),
            Color(0.54, 0.42, 0.16),
            0.80,
            0.08
        )
...
func _build_danger_area() -> void:
    ...
    danger_area.collision_mask = 1 | 4
    ...
    if hit_cooldown <= 0.0 and sweep_velocity.length() > 1.0:
        for body in danger_area.get_overlapping_bodies():
            if body.has_method("receive_hazard_hit"):
                var impulse := Vector3(sweep_velocity.x, 0.0, sweep_velocity.z).normalized() * 5.8 + Vector3.UP * 2.2
                body.receive_hazard_hit(12.0, impulse)
                hit_cooldown = 0.30
